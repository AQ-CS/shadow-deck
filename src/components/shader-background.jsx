...
/**
 * Neon Strands shader — plasma warp lines.
 * @param {{ color?: string }} props - Hex accent color (default: "#8b5cf6")
 */
const ShaderBackground = ({ color = "#8b5cf6" }) => {
  ...
  const fsSource = `
    ...
    void main() {
      ...
      // Use accent color for line color
      vec4 lineColor = vec4(uAccent, 1.0);

      vec4 lines = vec4(0.0);
      // Background: dark tinted version of accent
      vec4 bgColor1 = vec4(uAccent * 0.08, 1.0);
      vec4 bgColor2 = vec4(uAccent * 0.2, 1.0);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Pass accent color each frame from ref
      const c = colorRef.current;
      gl.uniform3f(info.uniformLocations.accent, c[0], c[1], c[2]);
      gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
